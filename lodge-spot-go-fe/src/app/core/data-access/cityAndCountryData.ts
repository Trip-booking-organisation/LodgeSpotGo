import { Address } from "../model/Address";

export const accomodationsAutoComplete: Address[] = [
    {
      "city": "Belgrade",
      "country": "Serbia",
      "street": "Knez Mihajlova"
    },
    {
      "city": "Chicago",
      "country": "United States",
      "street": "Michigan Avenue"
    },
    {
      "city": "New York",
      "country": "United States",
      "street": "Broadway"
    },
    {
      "city": "London",
      "country": "United Kingdom",
      "street": "Oxford Street"
    },
    {
      "city": "Paris",
      "country": "France",
      "street": "Champs-Élysées"
    },
    {
      "city": "Tokyo",
      "country": "Japan",
      "street": "Ginza"
    },
    {
        "city": "Dubai",
        "country": "United Arab Emirates",
        "street": "Sheikh Zayed Road"
      },
      {
        "city": "Sydney",
        "country": "Australia",
        "street": "George Street"
      },
      {
        "city": "Shanghai",
        "country": "China",
        "street": "Nanjing Road"
      },
      {
        "city": "Toronto",
        "country": "Canada",
        "street": "Yonge Street"
      },
      {
        "city": "Amsterdam",
        "country": "Netherlands",
        "street": "Damrak"
      },
      {
        "city": "Mumbai",
        "country": "India",
        "street": "Marine Drive"
      },
      {
        "city": "Cape Town",
        "country": "South Africa",
        "street": "Long Street"
      },
      {
        "city": "Rome",
        "country": "Italy",
        "street": "Via del Corso"
      },
      {
        "city": "Sarajevo",
        "country": "Bosnia and Herzegovina",
        "street": "Ferhadija"
      },
      {
        "city": "Moscow",
        "country": "Russia",
        "street": "Arbat Street"
      },
      {
        "city": "Rio de Janeiro",
        "country": "Brazil",
        "street": "Avenida Atlântica"
      },
      {
        "city": "Auckland",
        "country": "New Zealand",
        "street": "Queen Street"
      },
      {
        "city": "Seoul",
        "country": "South Korea",
        "street": "Myeongdong Street"
      },
      {
        "city": "Barcelona",
        "country": "Spain",
        "street": "Las Ramblas"
      },
      {
        "city": "Berlin",
        "country": "Germany",
        "street": "Unter den Linden"
      },
      {
        "city": "Istanbul",
        "country": "Turkey",
        "street": "Istiklal Avenue"
      },
      {
        "city": "Bangkok",
        "country": "Thailand",
        "street": "Sukhumvit Road"
      },
      {
        "city": "San Francisco",
        "country": "United States",
        "street": "Lombard Street"
      },
      {
        "city": "Buenos Aires",
        "country": "Argentina",
        "street": "Avenida 9 de Julio"
      },
    {
      "city": "São Paulo",
      "country": "Brazil",
      "street": "Avenida Paulista"
    },
    {
      "city": "Toronto",
      "country": "Canada",
      "street": "King Street"
    },
    {
      "city": "Hong Kong",
      "country": "China",
      "street": "Nathan Road"
    },
    {
      "city": "Athens",
      "country": "Greece",
      "street": "Ermou Street"
    },
    {
      "city": "Hanoi",
      "country": "Vietnam",
      "street": "Hang Bac Street"
    },
    {
      "city": "Dublin",
      "country": "Ireland",
      "street": "Grafton Street"
    },
    {
      "city": "Vienna",
      "country": "Austria",
      "street": "Kärntner Strasse"
    },
    {
      "city": "Stockholm",
      "country": "Sweden",
      "street": "Drottninggatan"
    },
    {
      "city": "Oslo",
      "country": "Norway",
      "street": "Karl Johans gate"
    },
    {
      "city": "Zürich",
      "country": "Switzerland",
      "street": "Bahnhofstrasse"
    },
    {
      "city": "Edinburgh",
      "country": "United Kingdom",
      "street": "Princes Street"
    },
    {
      "city": "Copenhagen",
      "country": "Denmark",
      "street": "Strøget"
    },
    {
      "city": "Lisbon",
      "country": "Portugal",
      "street": "Avenida da Liberdade"
    },
    {
      "city": "Brussels",
      "country": "Belgium",
      "street": "Rue Neuve"
    },
    // Add more accomodation objects as needed
  ]
  